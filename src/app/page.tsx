...
const activities = [
  { image: "/group-optimized.webp", title: "LEARNING & SKILLS", href: "/training", description: "Practical learning opportunities that build confidence, communication, wellbeing, rights awareness and enterprise skills." },
  { image: "/Nadia-optimized.webp", title: "WEEKLY DROP-INS", href: "/volunteering", description: "Welcoming sessions where women connect, share skills, access support and take part in activities led by volunteers and the community." },
  { image: "/wv-leadership-optimized.webp", title: "EVENTS & SOCIALS", href: "/about#activities", description: "Cultural events, creative sessions, shared meals and social gatherings that strengthen friendships, belonging and collective wellbeing." },
];
...
function WhatGuidesUs() {
  return <section id="priorities" className="wgu-section" aria-labelledby="what-guides-us-title"><div className="wgu-section-head wgu-reveal"><div><h2 id="what-guides-us-title">What Guides Us</h2></div><p>Our work is rooted in empowerment, equality, inclusion, wellbeing, education, and collective strength.</p></div><div className="wgu-policy-grid">{values.map((value, index) => <Link key={value.number} className="wgu-policy-card wgu-reveal" href="/about#guiding-principles" style={{ animationDelay: `${(index % 3) * 90}ms` }}><span>{value.number}</span><h3>{value.title}</h3><p>{value.description}</p><span aria-hidden="true" className="wgu-arrow">→</span></Link>)}</div></section>;
}

function ResearchSpotlight() { return <section className="home-research" aria-labelledby="research-spotlight-title"><div className="home-research-image"><Image src="/research-community.webp" alt="Women’s Voices presenting its community research" fill sizes="(max-width: 900px) 100vw, 50vw" /></div><div className="home-research-copy"><span>Community research · 2022</span><h2 id="research-spotlight-title">Community research, led by lived experience.</h2><p>Women from the community investigated the health and social needs of Black and minoritised women aged 50+ in Longsight, placing women’s own knowledge at the centre of the work.</p><div className="home-research-metrics" aria-label="Research highlights"><div><strong>50+</strong><span>Age group studied</span></div><div><strong>06</strong><span>Researchers trained</span></div><div><strong>09</strong><span>Key recommendations</span></div></div><Link className="home-research-link" href="/reports#community-research">Read the research →</Link></div></section>; }
function ConversationCallout() { return <section className="home-conversation" aria-labelledby="conversation-title"><span>Start a conversation</span><h2 id="conversation-title">Let’s make space for what comes next.</h2><div><Link className="home-conversation-button" href="/contact#enquiry-form">Send an enquiry <span aria-hidden="true">→</span></Link><a href={`mailto:${contact.email}`}>{contact.email}</a></div></section>; }

export default function Home() {
  return <>
    <section className="hero" aria-labelledby="home-title">
      <div className="hero-media-slider" aria-hidden="true">
        {heroSlides.map((src, index) => <div className="hero-media-slide" key={src} style={{ animationDelay: `${index * 5}s` }}><Image src={src} alt="" fill priority={index === 0} sizes="100vw" /></div>)}
      </div>
